...
public class ClienteDAO {
    ...
    public int obtenerIdPorDniOIdONombre(String idODniONombre) {
        String sql = "SELECT idUsuario FROM clientes WHERE idUsuario = ? OR dni = ? OR nombre LIKE ?";
        try (Connection con = ConexionSQL.getConexion();
             PreparedStatement ps = con.prepareStatement(sql)) {

            // Configurar parámetros para búsqueda
            ps.setString(1, idODniONombre); // Buscar por ID
            ps.setString(2, idODniONombre); // Buscar por DNI
            ps.setString(3, "%" + idODniONombre + "%"); // Buscar por Nombre (parcial)
            ...
            System.err.println("Error al obtener ID por DNI, ID o Nombre: " + e.getMessage());
            return -1;
        }
    }
